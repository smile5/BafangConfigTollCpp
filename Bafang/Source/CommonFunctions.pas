...
uses
  SysUtils, IniFiles, Main, Warning, Classes, Forms;
  ...
  function GetErrorText(errId: string): string;
  function GetErrorTitle(errId: string): string;
...
begin
   Result :='Unkwnow error message';
   errFile:=TMemIniFile.Create(ExtractFilePath(Application.ExeName) + 'Languages\' + selectedLang + '\soft\errors.ini');
   listL:= TStringList.Create;
   lineErr := errFile.ReadString ('ERRORS',errId ,'NULL');
   if lineErr<>'NULL' then
   begin
       ExtractStrings(['|'], [], PAnsiChar(lineErr) ,listL );
       Result:=listL.Strings[0];
   end;
...
var
  langFile: TInifile;
  nbLang: Integer ;
  i, indexDefault: Integer;
  langName: String;
  listL: TStrings;
begin
  indexDefault:=0;
  MainForm.cbbLanguage.Items.Clear ;
  langFile:=TInifile.Create(FileName);
  nbLang := langFile.Readinteger('LANGUAGES','nbLang',0);
  defaultLang := langFile.ReadString ('LANGUAGES','default','eng');
  listLang:= TStringList.Create;
  for i := 1 to nbLang do
  begin
    langName:= 'lang' + IntToStr(i);
    listL:= TStringList.Create;
    try
      ExtractStrings([','], [], PAnsiChar(langFile.ReadString('LANGUAGES',langName,'English,eng')),listL );
      MainForm.cbbLanguage.Items.Add(listL.Strings[0]);
      listLang.Add(listL.Strings[1]);
      if  listL.Strings[1]= defaultLang then
      begin
          indexDefault:=i-1;
      end;
    finally
  ...
  MainForm.cbbLanguage.ItemIndex := indexDefault;
  selectedLang:=listLang.Strings[indexDefault];
  langFile.Destroy; // Close file
end;
